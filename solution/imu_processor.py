...
def load_imu(path: str = "inputs/imu_data.npy") -> pd.DataFrame:
    arr = np.load(path)
...
def detect_cycles(df: pd.DataFrame, threshold: float = 30.0, min_distance_samples: int = 300):
    ...
    peak_indices = sorted(pos_peaks.tolist())
    cycle_times = []
    cycle_bounds = []
    for start_idx, end_idx in zip(peak_indices, peak_indices[1:]):
...
def compute_fatigue(df: pd.DataFrame, jerk_series: pd.Series, n_blocks: int = 3):
    ...
    for i in range(n_blocks):
        ...
        block_jerk = jerk_series.iloc[start:end]
        blocks.append(compute_smoothness(block_jerk))

    slope = 0.0
    if len(blocks) > 1:
        slope = (blocks[-1] - blocks[0]) / (len(blocks) - 1)
